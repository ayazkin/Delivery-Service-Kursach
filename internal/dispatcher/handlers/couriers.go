...
import (
	...
	"delivery-system/internal/dispatcher/kafka"
	"delivery-system/internal/dispatcher/models"
	"delivery-system/internal/dispatcher/services"
	"delivery-system/internal/platform/logger"
	"delivery-system/internal/platform/redis"
...
func (h *CourierHandler) UpdateCourierStatus(w http.ResponseWriter, r *http.Request) {
	...
	if !models.IsValidCourierStatus(req.Status) {
		writeErrorResponse(w, http.StatusBadRequest, "invalid courier status")
		return
	}
...
func (h *CourierHandler) DeleteCourier(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.courierService.DeleteCourier(courierID); err != nil {
		if strings.Contains(err.Error(), "not found") {
			writeErrorResponse(w, http.StatusNotFound, "Courier not found")
		} else if strings.Contains(err.Error(), "assigned orders") {
			writeErrorResponse(w, http.StatusConflict, "Courier has assigned orders")
		} else {
...
func (h *CourierHandler) validateCreateCourierRequest(req *models.CreateCourierRequest) error {
	...
	if len(req.Phone) > 20 {
		return fmt.Errorf("courier phone must be 20 characters or less")
	}
	return nil
}
