...
import (
	...
	"delivery-system/internal/dispatcher/database"
	"delivery-system/internal/dispatcher/models"
	"delivery-system/internal/platform/logger"
...
func (s *CourierService) UpdateCourierStatus(courierID uuid.UUID, req *models.UpdateCourierStatusRequest) error {
	if !models.IsValidCourierStatus(req.Status) {
		return fmt.Errorf("invalid courier status")
	}
...
// DeleteCourier удаляет курьера, если он не привязан к заказам
func (s *CourierService) DeleteCourier(courierID uuid.UUID) error {
	var hasAssignedOrders bool
	if err := s.db.QueryRow(
		"SELECT EXISTS(SELECT 1 FROM orders WHERE courier_id = $1)",
		courierID,
	).Scan(&hasAssignedOrders); err != nil {
		return fmt.Errorf("failed to check courier assignments: %w", err)
	}

	if hasAssignedOrders {
		return fmt.Errorf("courier has assigned orders")
	}

	result, err := s.db.Exec("DELETE FROM couriers WHERE id = $1", courierID)
	if err != nil {
	...
	s.log.WithField("courier_id", courierID).Info("Courier deleted")
	return nil
}
